...
class ServerAPI:
    ...
    def authorize_old(self, user:Dict[str, Any]):
        """
         Authorize a user. This is a POST request to the ` / web / user / authorize ` endpoint.
    ...
    def sync_screenshot_to_ralvie(self, object_key, record):
        try:
            ...
            payload = {"userId": userId, 
                        ...
                        "applicationName": record.event.app,           
...
class ScreenShotQueue(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while not self.should_stop():
            ...
            # จัดการเมื่อตื่นจาก Sleep / เพิ่ง Unlock หน้าจอหลังผ่านไปนานๆ
            if gap > SCREEN_SHOT_TIME * 2: 
                logger.warning(f"Significant time gap detected ({int(gap)}s). Machine woke up or unlocked. Resolving stale connections...")
                
                self.connected = False 
                self.wait(5) # รอ 5 วินาทีพอให้เน็ตเวิร์กเสร็จสิ้นการตั้งค่าใหม่
                last_tick = time.time()
